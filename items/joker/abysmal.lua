	...
	calculate = function(self, card, context)
		if (context.joker_main) or context.forcetrigger then
			return {
				...
				Xchips_mod = 1 / lenient_bignum(card.ability.extra.chips),
				colour = G.C.CHIPS,
			}
		end
		...
		if (context.setting_blind and pseudorandom("crp_bulgoe_bot") <
			cry_prob(card.ability.cry_prob, card.ability.extra.odds, card.ability.cry_rigged) / card.ability.extra.odds) or context.forcetrigger then
