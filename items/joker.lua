...
SMODS.Joker {
	...
	demicolon_compat = true,
	loc_vars = function(self, info_queue, card)
		return { vars = { card.ability.extra.emult, card.ability.extra.emult_gain, card.ability.extra.retriggers } }
	end,
	calculate = function(self, card, context)
		if context.repetition then
...
SMODS.Joker {
	...
	demicolon_compat = true,
	loc_vars = function(self, info_queue, card)
		return { vars = { card.ability.extra.mult, card.ability.extra.xmult, card.ability.extra.emult } }
	end,
	calculate = function(self, card, context)
		if context.setting_blind then
			card.ability.extra.mult = G.GAME.blind.chips
			card.ability.extra.xmult = G.GAME.round
			card.ability.extra.emult = G.GAME.round_resets.ante
			return {
				message = 'Upgraded!',
				colour = G.C.MULT,
				card = card
...
SMODS.Joker {
	...
	demicolon_compat = true,
	loc_vars = function(self, info_queue, card)
		return { vars = { card.ability.extra.chipsmult, '{', '}' } }
	end,
	calculate = function(self, card, context)
		...
		if context.individual and context.cardarea == G.play then
			...
			elseif arrow_number_cards == 2 then
				return {
					message = '^^' .. lenient_bignum(card.ability.extra.chipsmult) .. ' Chips & Mult',
					colour = G.C.EDITION,
					EEchip_mod = card.ability.extra.chipsmult,
			...
			elseif arrow_number_cards == 3 then
				return {
					message = '^^^' .. lenient_bignum(card.ability.extra.chipsmult) .. ' Chips & Mult',
					colour = G.C.EDITION,
					EEEchip_mod = card.ability.extra.chipsmult,
			...
			elseif arrow_number_cards >= 4 then
				return {
					message = '{' .. arrow_number_cards .. '} ' .. lenient_bignum(card.ability.extra.chipsmult) .. ' Chips & Mult',
					colour = G.C.EDITION,
					hyperchip_mod = {arrow_number_cards, card.ability.extra.chipsmult},
					hypermult_mod = {arrow_number_cards, card.ability.extra.chipsmult}
				}
			end
		end
	end,
